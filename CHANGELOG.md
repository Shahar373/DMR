...
## [0.14.0] - שכבת ה-Data: המפה שהייתה ריקה מבנית, הודעות טקסט, ורישום רדיו

**כל מה שנדרש היה כבר בקליטה שלנו — ונזרק.** שתי שורות שסולקו כ-housekeeping
(`SRC(24):` / `DST(24):`) נושאות את ה-RID של השולח והיעד **ואת פורט ה-UDP**,
והפורט הוא מסווג סוג-הדאטה בחינם (`dmr_pdu.c:345-484`). בלעדיהן, מפת ה-LRRP
של Phase 3 לא הציגה מעולם אף נקודה — לא כי הרשת לא שולחת מיקום, אלא כי זרקנו
את השורה שמכילה את מי-שלח.

### Fixed
- **★ `/api/positions` היה ריק מבנית — לא מחוסר LRRP.** `_RE_LRRP_POS` הכיל
  קבוצת `src` אופציונלית ש**מתמטית לא יכלה להתאים**: `dmr_pdu.c:844-858` בונה
  את המחרוזת `LRRP SRC: N; (lat, lon)` אבל מדפיס אותה רק ב-`if (!lat)` — כלומר
  שורה עם קואורדינטות לעולם לא נושאת SRC. לכן כל מיקום יצא עם `src=None`,
  ו-`_lrrp_snapshot` (שמסנן `src is None`) סינן **100%** מהמיקומים. ה-RID מגיע
  עכשיו משורת ה-`SRC(24)` שקדמה לו באותו PDU. הקואורדינטות בפיקסצ'ר שלנו
  (`Lat: 32.09265 Lon: 34.86761`) פוענחו כל הזמן — התיעוד שתלה את הריקנות
  ב-"Motorola proprietary לא מפוענח" היה **שגוי**, ותוקן.

### Added
- **★ סיווג PDU לפי פורט UDP** (`dsd_pty.DATA_PORT_KINDS`): 4001=מיקום (LRRP,
  ✅ מאומת בקליטה שלנו), 4005=רישום/נוכחות (ARS), 4007=הודעות טקסט (TMS),
  231=Cellocator, 4004=ניהול-רדיו, 4008/4009/4012/4013=טלמטריה/תכנות-מרחוק/
  סוללה/משימות. הכרטיס מקבל `data_kind`/`data_port`, וסוג הכרטיס נגזר מהם
  (`sms`/`reg`/`lrrp` — ערכים שכבר היו ב-`DMR_CALL_TYPES` וחיכו). פורט לא-מוכר
  ⇒ `kind=None`, בלי סיווג מומצא.
- **★ הודעות טקסט (TMS) עם תוכן.** `Text: ` נקלט ומוצג כ-`call_type="sms"`.
  **דלוק כברירת מחדל** — זו תעבורה שהרשת משדרת בגלוי; כיבוי דרך
  `DMR_CAPTURE_TEXT=0` ב-`dmr-web.env`, שמשאיר את המטא-דאטה (מי→מי, פורט)
  ומשמיט את התוכן בלבד.
- **★ כרטיס לרישום ARS ולסוגי PDU חסרי-payload** (ARS/טלמטריה/OTAP/סוללה/
  משימות/ניהול-רדיו): שורת ה-DST היא כל מה שנדע עליהם אי-פעם, והיא בכל זאת
  אירוע אמיתי (איזה רדיו, מול מי, איזה שירות). ל-lrrp/text יש payload שמייצר
  את הכרטיס, ולכן הם **לא** מייצרים כרטיס כאן — אין כפילות.
- **שדות LRRP נוספים:** `fix_time`, `speed_kmh`, `track_deg`, `alt_m`,
  `radius_m` (`dmr_pdu.c:735-791`). ⚠ אף אחד מהם לא נצפה בקליטה שלנו — נגזרו
  מקוד-המקור ונוספו לפיקסצ'ר המופרד עם `provenance` (§7/§8). `Time:` מודפס
  **לפני** שורת ה-Lat/Lon והשאר **אחריה**, ולכן חלקם נכנסים כהקשר וחלקם
  כמוטציה על הכרטיס הפתוח — ומשום כך כרטיסי מיקום נדחים לכתיבה כמו שיחות-קול
  (אחרת חוזר בדיוק הבאג של v0.13.0: הארכיון קופא לפני שהמידע הצטבר).
- UI: תגי `✉ הודעה` / `📋 רישום` / `📊 טלמטריה` וכו' על הכרטיס, ותגי
  מהירות/כיוון/גובה על כרטיסי מיקום.

### Changed
- **⚠ סטייה מודעת מ-§1 שנשארת, ועכשיו מתועדת ומוצהרת ב-UI:** אריחי המפה
  נטענים מ-`tile.openstreetmap.org`. עד עכשיו זה היה תיאורטי (המפה תמיד
  הייתה ריקה); מהרגע שיש בה נקודות, כל צפייה חושפת לשרת חיצוני את אזור
  הקואורדינטות. נוספה אזהרה גלויה מעל המפה + תיעוד ב-CLAUDE.md §8. Leaflet
  עצמו נשאר vendored (בלי CDN).
- `SRC(24)`/`DST(24)` אינם housekeeping יותר — עודכנו ה-fixture replay
  (68/68 עדיין ירוק, עם alias מוצהר `ip_mapping`→`ip_data`) ובדיקת
  ה-housekeeping שקודם קבעה במפורש שהשורה הזו נזרקת.
- 314 בדיקות ירוקות (302→314). אומת גם חזותית (Playwright מול `app.py`
  אמיתי): חמישה סוגי כרטיסים חדשים + **המפה מציגה שני סמנים אמיתיים**
  בפעם הראשונה מאז ש-Phase 3 נבנה.
