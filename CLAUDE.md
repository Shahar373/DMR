...
> **הבינארי בפועל: `arancormonk/dsd-neo`** (לא `lwvmobile/dsd-fme`) — נתפס בהרצה
> ראשונה על חומרה אמיתית: ל-`dsd-fme` אין קלט `‎-i rtltcp:` שהגשר `rsp_tcp` דורש.
> "DSD-FME" במסמך הזה הוא שם המשפחה/הפרוטוקול הטקסטואלי (משותף לשני ה-forks),
> לא שם הבינארי הספציפי — ר' `install.sh` §10 ו-CHANGELOG לפרטים.
