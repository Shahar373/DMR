...
- **Phase 5 (v0.4.0, הושלם — אומת על חומרה אמיתית):** תוקן קריסת `dmr-dsdfme`
  ...
  **בדרך נוסה גם מעבר ל-`arancormonk/dsd-neo`** (fork אחר שתומך `‎-i rtltcp:`
  ...
  ומתחיל תהליך פענוח/טראנקינג. **★ הפער האחרון נסגר ב-28.07.2026 (v0.18.2)
  — ר' שם: שיחת-קול אמיתית (מוצפנת, מתויגת חירום) פוענחה מקצה-לקצה.**
...
- **★ v0.14.0 — "יש אות, אין פענוח": באג הדצימציה ב-multi + סוף העיוורון
  ...
  **הפענוח עצמו אומת על חומרה ב-28.07.2026 — ר' v0.18.2.**
...
- **v0.16.0–v0.16.2 — תיקוני-שטח (תדר-חתך, control-socket, כיוון-אינדקס):**
  ר' CHANGELOG. הביאו למדידה הראשונה שהחזירה `Sync: +DMR` יציב עם שיעורי-
  FEC נמוכים בהרבה, אבל עוד בלי שיחת-קול מלאה שנקלטה.
- **v0.17.0 — אפיון RF בסימולציה** (`test_rf_characterization.py`): מיפוי
  גבולות השרשרת (SNR/שגיאת-תדר/ערוץ-שכן/רצפת-כימות/חיתוך) לפני שממשיכים
  לנחש בשטח. ר' CHANGELOG לממצאים (במיוחד `scaled_taps` במרווח 12.5kHz).
- **v0.18.0–v0.18.1 — `webtune/rf_probe.py` + RUNBOOK.md:** כלי-CLI
  לאבחון שטח (capture/gain-sweep/analyse) שמודד פקיחת-עין ושיא-ספקטרום
  מ-IQ גולמי, ומדריך-הרצה מובנה כחצייה בינארית. ר' §7/CHANGELOG.
- **★★ v0.18.2 (28.07.2026) — ההוכחה: פענוח קצה-לקצה על חומרה אמיתית,
  לראשונה מאז v0.4.0.** בסשן-השטח הראשון עם `rf_probe`/RUNBOOK נקלטה
  ונפענחה שיחת-קול **אמיתית** — TG 614369, RID 4370742, שיחת-קבוצה,
  TS1, **מוצפנת**, מתויגת **חירום** (Service-Option שתוקן ב-v0.13.0
  אחרי שנדחה בשקט) — הכרטיס עבר את כל השרשרת: RF→demod→DSD-FME→
  `parse_dsd_line`→`_normalize_dsd`→`dmr.jsonl`→UI. זו התשובה לשאלה
  הפתוחה מאז Phase 5 ("איכות הפענוח בפועל ממשיכה להיבדק בשטח").
  שני ממצאים נוספים מאותו סשן:
  1. **ה-AGC של `rsp_tcp` נעל ברווח מינימלי בפועל.** מדידה עם `rf_probe`:
     AGC → front ‎−34 dBFS; רווח ידני `index=24` → **‎−19.8 dBFS** (+14dB!),
     `clip_frac`=0. `connect()` ב-`RtlTcpClient` (`rsp_fm.py`) מדליק AGC
     תמיד ב-startup, ואין כרגע env-var לרווח-פתיחה ידני. **לא שונה
     עדיין** — נדרשת סריקה מלאה (`gain-sweep`) לפני שקובעים ברירת-מחדל
     חדשה; ר' "הבא" למטה.
  2. **multi: הפיזיקה תואמת את הסימולציה בול.** מתוך `multi_164cluster`
     החי: LCN3 (164.325, CC6 "גבוה מאוד" בסקר) חזק ביותר (‎−19..‎−20 dBFS)
     ו**הכי נקי** (22 שגיאות/60ש', הכי-נמוך מבין 4 הערוצים שנעלו); LCN1
     ו-LCN5 (‎−48..‎−52 dBFS) מתחת לרצפת-הכימות ולא נעלו כלל — בדיוק
     כפי ש-`test_rf_characterization.py` צפה. **164.325, לא 164.3, הוא
     הערוץ הנכון לכל בדיקת-יחיד עתידית.**
  3. **⚠ אזהרת `undervoltage` נצפתה** (4.77V, שהעליון-מחייב ל-Pi 5 +
     RSP1B הוא 5.1V) — מועמד לגורם-שורש חלקי לעוצמות-הכניסה הנמוכות
     שנמדדו בכל הערוצים. טרם נבדק ישירות (ספק/כבל).
...
- **★ פתוח מ-v0.18.2 — רווח-פתיחה ידני כברירת-מחדל במקום AGC:** מדידת
  שדה בודדת הראתה +14dB front-level עם `index=24` לעומת AGC (ר' v0.18.2
  למעלה), אבל זו נקודת-מדידה אחת, לא סריקה מלאה. **לפני שינוי ברירת-
  המחדל:** (1) להריץ `rf_probe gain-sweep` מלא על 164.325 (הערוץ הנקי
  שהתגלה) ולוודא שהעקומה מונוטונית ואינה רק רעש-רגע; (2) לשלול/לאשר את
  ה-`undervoltage` כגורם-שורש נפרד (ספק/כבל) לפני שקובעים גיין קבוע סביב
  תקלת-חומרה זמנית. אם שניהם מאששים — נדרש env-var חדש (למשל
  `DSD_INITIAL_GAIN`) שמועבר מ-`render_dmr_env` דרך `dsd_pty` ל-
  `RtlTcpClient.connect()`, כי כרגע `connect()` מדליק AGC תמיד וללא
  חלופה מוגדרת-קונפיג.
