...
- **Phase 5 (v0.4.0, ⚠ ממתין לאימות על חומרה אמיתית):** תוקן קריסת `dmr-dsdfme`
  ...
  **טרם אומת על RSP1B אמיתי** — כל השרשרת `rsp_tcp→rsp_fm.py→DSD-FME` היא
  `pragma: no cover` (תלוית-hardware, לא נבדקת ב-CI); pytest ירוק מוודא רק את
  הלוגיקה הטהורה (argv, פרסור, דמודולטור מול אות מסונתז). לפני שמסמנים Phase 5
  כ"הושלם" — ודאו הרצה אמיתית: השירות נשאר יציב (לא קורס תוך שניות/דקות) ומפענח
  שיחות אמיתיות מרשת Cap+.
